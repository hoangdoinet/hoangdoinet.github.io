...
function updateNetworkUI() {
  if (!el.connectionStatus || !el.connectionWrapper) return;
  
  const oldIcon = document.getElementById('connection-icon');
  if (oldIcon) oldIcon.remove();

  if (navigator.onLine) {
    el.connectionStatus.textContent = "Đã kết nối máy chủ an toàn";
    el.connectionWrapper.classList.remove('text-amber-200', 'bg-amber-900/40');
    el.connectionWrapper.classList.add('text-teal-100/90', 'bg-white/10');
    ...
    el.connectionWrapper.classList.remove('text-teal-100/90', 'bg-white/10');
    el.connectionWrapper.classList.add('text-amber-200', 'bg-amber-900/40');
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    try {
      ...
      // Gọi cập nhật trạng thái UI mạng
      updateNetworkUI();
...
window.addEventListener('offline', () => {
  if (currentUserUid) updateNetworkUI();
});
