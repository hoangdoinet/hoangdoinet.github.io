...
const CACHE_STATIC = `timestamp-edit-${APP_VERSION}`;
...
const CRITICAL_ASSETS = [
  ...
  BASE + 'libs/piexif.min.js'
];

console.log(`[SW ${APP_VERSION}] 🚀 Khởi động - Timestamp Edit Pro`);
...
self.addEventListener('install', (event) => {
  console.log('[SW] 🔧 Đang cài đặt tài nguyên cho Timestamp Edit Pro...');
  event.waitUntil((async () => {
    ...
    if (okMinimal) {
      ...
      console.error('[SW] 🚨 Thiếu bộ thư viện cốt lõi → Không bỏ qua chờ đợi (skipWaiting) để tránh lỗi ứng dụng.');
    }
  })());
});
...
self.addEventListener('activate', (event) => {
  console.log('[SW] 🎯 Kích hoạt - Xóa bộ nhớ đệm cũ...');
  event.waitUntil((async () => {
    if (self.registration.navigationPreload) {
      try { await self.registration.navigationPreload.disable(); } catch {}
    }

    // Xoá bộ nhớ đệm thuộc phiên bản cũ của ứng dụng này
    const keys = await caches.keys();
    await Promise.all(
      keys.filter(k => k.startsWith('timestamp-edit-') && k !== CACHE_STATIC).map(k => caches.delete(k))
    );
    ...
    const critical = [BASE + 'index.html', BASE + 'libs/tailwind.min.js', BASE + 'libs/lucide.min.js', BASE + 'libs/piexif.min.js'];
...
<head>
  ...
  <title>Timestamp Edit Pro - Offline</title>
  <style>
...
<body>
  <div class="container">
    ...
    <p>Không tìm thấy dữ liệu bộ nhớ đệm. Ứng dụng xử lý ảnh Timestamp Edit Pro cần kết nối mạng trong lần đầu truy cập để thiết lập cấu trúc chạy Offline hoàn toàn.</p>
...
console.log(`[SW ${APP_VERSION}] ✅ Đã tải - Sẵn sàng phục vụ Timestamp Edit Pro`);
